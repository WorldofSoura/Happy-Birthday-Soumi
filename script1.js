...
const photoObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            ...
            if (img) {
                img.style.animation = 'photoEnter 0.5s ease-out forwards';
            }
        }
    });
}, { threshold: 0.2 });
...
function startCelebration() {
  scrollToSection('gallery'); // still scrolls
  ...
  if (!isPlaying) {
    audio.play();
    button.textContent = "⏸ Pause the Celebration 🎶";
    isPlaying = true;
  } else {
    audio.pause();
    button.textContent = "▶ Resume the Celebration 🎉";
    isPlaying = false;
  }
}
// function playSong() {
